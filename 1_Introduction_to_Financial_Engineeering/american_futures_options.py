...
class BinomialPricer:
    def __init__(
        self, S0: float, K: float, T: float, n_periods: int, r: float, sigma: float, c: float
    ):
        ...
        self.dt: float = T / n_periods
        self.u: float = np.exp(sigma * np.sqrt(self.dt))
        self.d: float = 1.0 / self.u
        self.p: float = (np.exp((r - c) * self.dt) - self.d) / (self.u - self.d)
        self.df: float = np.exp(-r * self.dt)

    def price_stock_option(self, option_type:str='call'):
